define(function(require) {
	...
	var Mcq = QuestionView.extend({
		...
		postRender: function () {
			QuestionView.prototype.postRender.call(this, arguments);

			this.$('.buttons-action').on('click', _.bind(function () {
				Adapt.once('questionView:showFeedback', this.patchFeedbackUI);
			}, this));
			this.$('.buttons-feedback').on('click', _.bind(function() {
				Adapt.once('questionView:showFeedback', this.patchFeedbackUI);
			}, this));
		},

		patchFeedbackUI: function () {
			Adapt.once('popup:opened', function(event, data) {
				$('.notify-popup').addClass('notify-kc-popup')
			});
		},
